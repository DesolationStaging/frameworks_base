...
import com.android.internal.util.desolation.DesoUtils;
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private void interceptScreenrecordChord() {
        if (mScreenrecordChordEnabled
            ...
            final long now = SystemClock.uptimeMillis();
            if (now <= mScreenrecordChordVolumeUpKeyTime + SCREENRECORD_CHORD_DEBOUNCE_DELAY_MILLIS
    ...
    Runnable mBackLongPress = new Runnable() {
        public void run() {
            if (DesoUtils.killForegroundApplication(mContext)) {
                performHapticFeedbackLw(null, HapticFeedbackConstants.LONG_PRESS, false);
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (mScreenrecordChordEnabled && (flags & KeyEvent.FLAG_FALLBACK) == 0) {
            if (mVolumeUpKeyTriggered && !mPowerKeyTriggered) {
                final long now = SystemClock.uptimeMillis();
                final long timeoutTime = mScreenrecordChordVolumeUpKeyTime + SCREENRECORD_CHORD_DEBOUNCE_DELAY_MILLIS;
                if (now < timeoutTime) {
                    return timeoutTime - now;
                }
            }
